<script setup lang="ts">
import { useRouter } from 'vue-router'
import { platformLabel } from '@/api/platforms'
import Button from '@/components/ui/Button.vue'
import { ArrowLeft, Gamepad2, Image, Save, Clock, BookOpen, Cpu } from 'lucide-vue-next'

const props = defineProps<{ tag: string }>()
const router = useRouter()

const resources: { key: string; label: string; icon: typeof Gamepad2; disabled?: boolean }[] = [
  { key: 'roms', label: 'ROMs', icon: Gamepad2 },
  { key: 'saves', label: 'Saves', icon: Save },
  { key: 'states', label: 'Save States', icon: Clock, disabled: true },
  { key: 'bios', label: 'BIOS', icon: Cpu },
  { key: 'art', label: 'Box Art', icon: Image },
  { key: 'guides', label: 'Guides', icon: BookOpen },
]
</script>

<template>
  <div class="mx-auto max-w-6xl p-6 space-y-8">
    <div class="flex items-center gap-3">
      <Button variant="ghost" size="icon" @click="router.push({ name: 'dashboard' })">
        <ArrowLeft class="h-5 w-5" />
      </Button>
      <h1 class="text-2xl font-bold tracking-tight">{{ platformLabel(props.tag) }}</h1>
    </div>

    <div class="grid grid-cols-3 gap-4">
      <div
        v-for="res in resources"
        :key="res.key"
        class="rounded-xl border border-border bg-card p-8 flex flex-col items-center gap-4"
        :class="res.disabled ? 'opacity-50 cursor-default' : 'group cursor-pointer hover:border-accent/50 hover:shadow-md hover:shadow-accent/5'"
        @click="!res.disabled && router.push({ name: 'browse', params: { resource: res.key, tag: props.tag } })"
      >
        <div class="flex items-center justify-center h-14 w-14 rounded-xl bg-muted group-hover:bg-accent/10">
          <component :is="res.icon" class="h-7 w-7 text-muted-foreground group-hover:text-accent" />
        </div>
        <span class="font-semibold text-lg group-hover:text-accent">{{ res.label }}</span>
        <span v-if="res.disabled" class="text-xs text-muted-foreground -mt-2">Coming soon™</span>
      </div>
    </div>
  </div>
</template>
